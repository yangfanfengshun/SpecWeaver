...
description: 在开发 Tower 或本地需求资料时通过 SpecWeaver 缓存清单发现已确认设计，先查看项目内蓝湖预览图，再按组件或区域查询缓存中的规范化设计事实并据此还原页面。用户要求“开发这个 Tower 需求”“按需求文档实现”“照设计稿还原页面”或修改带有蓝湖来源资料的前端界面时使用。负责设计证据消费与视觉回归，不负责重新收集需求。
---
...
1. 从用户给出的 Tower ID、需求名称、`requirement.md` 路径或当前任务目录定位需求
   资料目录，从 `tower-raw.md` 读取 Tower URL，并调用
   `requirement_get_manifest(tower_url, output_dir)` 定位用户缓存中的收集清单。
2. 只有一个匹配项时自动采用，不要求用户重复提供蓝湖链接。
3. 多个匹配项无法根据 Tower ID 或路径排除时，列出候选并暂停。
4. 读取清单中的 `lanhu.items`，验证预览图、结构文件和切图目录真实存在；缺失时
   报告具体文件，不伪造。
