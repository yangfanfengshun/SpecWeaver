...
description: 从 Tower 读取任务元信息、正文、独立评论、子任务标题、附件索引和外部来源线索，缓存正文与评论图片，确定性生成 tower-raw.md、tower-metadata.json 和图片目录并返回小型摘要。仅在用户明确要求只读取或核对 Tower 原文、总控流程内部预读 Tower、或旧任务明确点名本 Skill 时使用。普通 Tower 需求入口由 requirement-collection 处理。不选择模式、不读取其他平台、不生成需求分析文档。
---
...
1. 验证输入是 Tower 任务链接。
2. 调用 `tower_read_todo(url)`；工具必须同时缓存正文和全部评论中的普通图片。
