...
6. 让用户直接在终端向导中输入 Cookie、账号和密码；不要让用户把密钥发送到对话中。
7. 按终端汇总说明成功、已关闭、已配置待首次使用验证或失败的平台。验证失败时只重试失败平台。

蓝湖安装和配置不需要项目链接。蓝湖 Cookie 保存后标记为“已配置，待首次使用验证”；首次处理真实蓝湖链接时，再由蓝湖 MCP 检查 Cookie 和项目权限。

## 非交互模式

仅当用户明确要求自动化配置，并且必要环境变量已在终端环境中准备好时，运行 `specweaver configure --non-interactive`。可以在命令末尾指定单个平台；未指定时检查全部已启用平台。`LANHU_CHECK_URL` 只用于用户明确要求提前检查某个蓝湖项目权限的场景，不是安装必填项。
